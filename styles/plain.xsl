...
<!-- END <xsl:import href="common/common.xsl"/> -->
    ...
    <xsl:template match="tei:TEI">
        <h1>Text: <xsl:value-of select="@xml:id"/>
        </h1>
        <h2>
            <xsl:for-each select="tei:teiHeader//tei:titleStmt/tei:title">
    ...
    <xsl:template match="cq:corpusHeader/tei:teiHeader|cq:textHeader/tei:teiHeader">
        <h1>This style is not applicable to
  <xsl:choose>
                <xsl:when test="../cq:corpusHeader">corpus</xsl:when>
                <xsl:when test="../cq:textHeader">text</xsl:when>
            </xsl:choose> headers, please select 'voice'</h1>
    </xsl:template>
...
<!--   </xsl:call-template> -->
    ...
    <xsl:template match="tei:u">
        ...
        <div class="{$u_classes}" id="{$uid}">
            <span class="ctx_activator jslink">
                <xsl:text>▾</xsl:text>
            </span>
            <span class="{$sid_classes}" id="src_{$uid}_{substring-after(@who,'#')}">
